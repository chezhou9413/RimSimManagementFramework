...
namespace RimSimRestaurantExtension.Debug
{
    ...
    {
        //在鼠标点选位置生成闭环自助餐厅，职责是提供运输、厨师补餐与正常收银的调试场景。
        [DebugAction("SimRestaurant", "生成传送带餐厅（闭环与食品）", false, false, false, false, false, 0, false,
            actionType = DebugActionType.ToolMap,
            allowedGameStates = AllowedGameStates.PlayingOnMap)]
        private static void SpawnConveyorRestaurantAtCell()
        {
            if (!CompleteRestaurantCreationUtility.TryCreateConveyorRestaurant(Find.CurrentMap, Verse.UI.MouseCell(),
                    out Zone_Shop shop, out string result))
            {
                Messages.Message("生成传送带餐厅失败：" + result, MessageTypeDefOf.RejectInput, false);
                return;
            }
            Messages.Message("已生成传送带餐厅：" + result + "。", MessageTypeDefOf.TaskCompletion, false);
        }
