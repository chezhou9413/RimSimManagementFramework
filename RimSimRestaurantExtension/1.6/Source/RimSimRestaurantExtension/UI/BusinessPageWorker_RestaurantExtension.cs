...
namespace RimSimRestaurantExtension.UI
{
    ...
    public class BusinessPageWorker_RestaurantExtension : BusinessManagerPageWorker
    {
        ...
        {
            if (state.lastSearch != context.SearchText)
            { state.lastSearch = context.SearchText; state.page = 0; state.scroll = Vector2.zero; }
            var sessions = all.Where(s => state.includeHistory || !s.IsTerminal).Where(s => Matches(context.SearchText, CustomerName(s.Anchor) + " " + s.reason
            ...
            try
            {
                ...
                foreach (var session in shown)
                {
                    ...
                    { DrawOrder(new Rect(18, y, view.width - 18, childHeight), order, order.orderId, line, session.selfService); y += childHeight; }
                }
            }
        ...
        private static void DrawOrder(Rect rect, RestaurantOrder order, int index, float line, bool selfService)
        {
        ...
        {
            ShopUiVisualUtility.DrawCellLabel(rect, text);
        }
