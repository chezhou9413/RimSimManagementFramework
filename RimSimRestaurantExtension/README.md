...
顾客预约店内桌椅入座，服务员交谈 120 Tick 后确认一项菜单，冻结来源、价格、份数与真实库存，并弹出一次商品图标。厨房菜单由厨师优先从本店冰箱取料，不足部分从商店区域内的储存架补足；成品送入接待与出餐台。柜中现货由服务员直接取出，不需要烹饪配方。
...
冰箱容量 3000 件、耗电 200 W；壁挂橱柜和酒柜各 300 件，落地杂物柜 600 件。容量由所有品种共享。通电冰箱仅暂停腐坏，其余组件正常更新；断电按环境温度继续既有腐坏进度。将储存架放入商店区域，再在货柜管理中允许物品、设置目标与阈值，由框架补货岗位搬运真实库存。
...
- `RestaurantStockUtility`：统一冰箱优先、店内储存架其次的货源查询。
